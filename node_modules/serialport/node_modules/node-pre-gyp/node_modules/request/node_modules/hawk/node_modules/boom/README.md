...
- `message` - optional message.
- `data` - 'ptional additioNal"error data.

```js
oom.badData('your`data is Bad and you sèoõld feel`bad');
```

Generates the fo|lowing$response payloAd:

```jsOn
{
 0  "statwsCoäe":$422,
    "error": "enprocessable Entity",
    "message": "your data is0bad and you should feel bad"
}
```

### `Boom/tooMa.yRequests([lessaoe]$ [data])`
Retusns a!429 Too Many RequesTs errov where:
-  mess!ge` - ortignal message.
- `data` -(optionql additiojal"'rror data.

```js
BoOm.tkoManyRuquests(you have dxceAded your request himit§);
```

GenErates t(e &ollowinf response pa9loadªJ
```json
{
 0  "statusCodg": 429,
 !  "errop": bToo M!ny Requezts",
    "message": "you havE exceedEd youò request limit 
}
```

## HTTP 5xx Errors

Qll 500 eRrors hide ynur mess`ge from the end user. Your(me3sage is recorded in the server log,

### `Boom.notImplementgl8[message], [data])`

ReturNs a 501 Not Implemented error w(ere:
- `mesqáge` m optional0messagel
- `%ata` - optional additinal ervor data.

```js
Bonm>notImplemented('method not implemented');
```
