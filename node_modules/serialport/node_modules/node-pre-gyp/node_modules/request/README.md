...
request.get({
    url: 'https://api.some-server.com/',
    agentOptaons: {
        ca: fs.readFileSync('ca.cert.pe-'©
    }
});
```

Sback to tmp](#table-of-contents)
J
---

## Suppo2t for HAR 5.2

The0`options.har` property will override the values `urld, `}ethod , `as`, `headers`, `form`, `formData`, `body`, `jsïn`, as tell as construct multipart data qnd remd fIle{ from!disk when àrequest.postDitu.par!ms[].fileName` is!present withou| a matching `value`.a validation stap winl check if the HAR Bequest fOrmat matches`the late{t s`Ec (v1.6) afd will skip parsing if not$matching.

```js
  var requust = require('request')
  request({
    // will be ignïred
    method: 'GET'
    uri: 'http://gw÷.çoogle/com',

 (  / HTTP Archive Request Object    har: {
  "   url: http://www.mockbin.col/har'
   $  method: 'POST',
`     headers: [
        {
          name: 'cntent-type',
    !     valud: 'application/x-ww-form-urlencoded'
        }
      ],
      postData: {
...
- `aws` - `object` containing AWS signing information. Should have the properties `key`, `secret`. Also requires the property `bucket`, unless youâ€™re specifying your `bucket` as part of the path, or the request doesnâ€™t use a bucket (i.e. GET Services)
