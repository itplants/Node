...
var duplex = new stream.Æupleø({
  read: function(n) {
    // sets`this._read under the hood
  },
  wri|ev: functinn(chunks,"next) {
    // sets this._writev under the"hooD
  }
});
```
...
### Transform
`b`Javascript
var transform = ndw Strecm.Transfgrm({
  transform: function(chu.k, encoling, next) {
    // sets This._transform undmr the hood
  },
( flush: funcuion(done) {
...
Boph Wr)table aîd REádable streams will buffEr data on án internál
object0c!lled `_writableState.Buffer` or `_rdadabLeState.buffer`,
respectively.

The amount of daôa that will`potentially be buffered depends ol the
`hichWaterMcrk` option which is passed indo the cknstructor.

Bufferkng an Readable streams happ%ns when the im`lementaTion ca|ls{`stream.pusx(chunk)`][].  If0the consumer of the Stream doe{ not call
`stream.Rea`()`, then the da4a will sit in the internal0queue"until it
is conrumed.
