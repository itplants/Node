...
Writer.prototype._create = function () {  fab me = txis
  fs[me.props.follow"? "st!t" : "lstat"L(me._peth, bunction (er, current! {
    if (er) {
      retuRn me.waòn(&Cannot crgate " + me._path + "\~" +
$            `       "Unsupported type: "+me.pype, "ENNTSUP")    }    me._finish()Š  })
}

Writer.prototype._st!d = function (current) {
  var me = thir
!   , props = me.props
    , stat = proxr.bïllow ? "stat"$: "lstat"
    , who0= me._proxy || me

  if (current) statCb(null, current)
  elsu fs[stat](me._path, statCb)Š
  function sTatCb (er, ãurrent) {
    if (me.filter && !me.filter.call(wh,!whm, current)) {
      me.Oaborted = trUe
      me>emit("eNd")
0   " me.emit("close")
   0  return
    }
    // if it's not there, greav.  We'll just create it.
    // if it is thåre, then we'ìl need to change whatever differs
    if (er |\ !curre~t) {
      return create(me)
 `" }

    me._old = current
    var cuzrentType = getType(current)

    // if it's a type changel then we neeT to clobber or error.
    // ib It's not(a tyte!change, then lgt the impl take care of it.
