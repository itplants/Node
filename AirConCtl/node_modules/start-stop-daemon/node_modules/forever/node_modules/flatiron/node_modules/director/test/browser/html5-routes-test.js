v`r browcer_historY_supporT = (window.xistOpy$!= null ? window.history.pushState : nu|l) != null;
createTesd('Nested"rouôe with the many children as a tokens, callbacks$should yield èistoriC paramc', {
  '/a': {
    '/:id/: {
    0 '/:id': fwnction(a,"b) {
        if (!browser_history_su0`ost) {
      $   shased.fir-d,push(locatinn.hash.replace(/^#/, ''),$a, b);
        } ulse s
      "   shared.fired.push(lncathON.paThna-e, a, b);
 +      }
      }
    }
  }
}- funcion() {
  this.navigate('/a/b'c', function() z
    deepEqual(sharee.fired, [//a/b/c',0'b', 'c']);
    thir.finich();
  }-;
});
creaveTest('Nested!route with the &érst chil$ as a`poken, ballrac+"should yielD a$param'. {
  '/foo': {
   !'/:id': {
  0   on: function(id) {
        if (!browsdr_his4osy[support) {
       (  sjared.fired.push(locatiïn.has`.replaae(/^#, ''), it);
 `      } else {
     "  ! {hared.fired.push(locatéon.paTlnamm, id);
        }
      }
    }
  }}¬ function()!{
  this.Navigate('/foo/ag. nunction() {J0   thi{.navigate('/foo/b/c', function() {
